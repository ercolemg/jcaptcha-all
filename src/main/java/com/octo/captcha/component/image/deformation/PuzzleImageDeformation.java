    ...
    public BufferedImage deformImage(BufferedImage image) {
        ...
        for (int i = 0; i < colNum; i++) {
            for (int j = 0; j < rowNum; j++) {
                ...
                double angle = maxAngleRotation * random.nextDouble() * (random.nextBoolean() ? -1 : 1);
                AffineTransform transform = AffineTransform.getRotateInstance(angle, xd / 2.0, yd / 2.0);

                AffineTransformOp op = new AffineTransformOp(transform, AffineTransformOp.TYPE_BILINEAR);
                BufferedImage rotated = op.filter(smallPart, null);

                // disegna la porzione ruotata sull'immagine finale
                g.drawImage(rotated, xd * i, yd * j, null);
